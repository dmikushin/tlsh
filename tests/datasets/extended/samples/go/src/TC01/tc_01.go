...
func makeHumanReadable(size int64) string {
	if size < 1024 { // less than 1 KB
		...
		return strconv.FormatInt(size/1048576, 10) + "MB"
	} else { // Bigger than 1GB
		return strconv.FormatInt(size/1073741824, 10) + "GB"
	}
}
...
func main() {
	...
	if !*showHiddensPtr {
		directoryFileNames = removeHiddenFiles(directoryFileNames)
	}

	if *longListPtr {
		showLongList(directoryFileNames, directoryAddress, *humanReadablePtr)
	} else {
